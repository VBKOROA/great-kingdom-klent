//! Pure game rules, board representation, and rule primitives.
...
impl GameState {
    #[must_use]
    pub const fn current_player_value(&self) -> Player {
        self.current_player
    }
}

/// Builds a state directly from a board layout. Intended for tests and
/// deterministic reconstruction, not for production game flow.
#[must_use]
#[doc(hidden)]
pub fn state_with_board(board: [Cell; BOARD_CELLS], current_player: Player) -> GameState {
    GameState {
        board,
        current_player,
